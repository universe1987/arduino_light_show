...
LightEffect* uniform_dynamic = new UniformDynamicEffect();
LightEffect* signal_transmission = new SignalTransmissionEffect();
LightEffect* pulse = new PulseEffect();
LightEffect* spiral = new SpiralEffect(2, 30);
LightEffect* flame = new FlameEffect(38, get_intrinsic_level);
LightEffect* burst = new BurstEffect();
...
void loop() {
  ...
  my_light.set_effect(uniform_dynamic, 30, 10);
  my_light.set_effect(signal_transmission, 30, 20);
  color_scheduler.set_cycle(120);
  my_light.set_effect(pulse, 30, 16);
  color_scheduler.set_cycle(30);
  my_light.set_effect(burst, 30, 20);
  // Spiral effect looks better under discrete mode
  color_scheduler.set_discrete_mode();
  my_light.set_effect(spiral, 30, 15);
  my_light.set_effect(flame, 30, 30);
}
